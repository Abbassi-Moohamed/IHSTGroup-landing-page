...
function Scene({ targetRef, themeKey }: { targetRef: React.RefObject<number>; themeKey: "dark" | "light" }) {
  ...
  useFrame(() => {
    ...
    if (groupRef.current) {
      groupRef.current.position.y = THREE.MathUtils.lerp(2, -3, p);
      groupRef.current.rotation.x = THREE.MathUtils.lerp(0.04, -0.04, p);
...
export function NodesBackground({ containerRef, theme = "dark" }: NodesBackgroundProps) {
  ...
  return (
    ...
    >
      <Canvas
        ...
        camera={{ position: [0, 0, 22], fov: 55, near: 0.1, far: 60 }}
        gl={{
          alpha: true,
          antialias: false,
